...
compose in the cascade instead of multiplying into authored combinations. There are five: color mode,
high contrast, density, radius, and accent reach.

**Accent reach**:
A dimension. How MANY places wear the brand colour, never how saturated it is: at every level the
accent is the same colour, and what changes is the count of things reaching for it. Three levels
(`data-accent`): 3 is today's spread, 2 takes it off navigation chrome and decoration, 1 takes it off
prose links too. Expressed as reach tokens (`--color-nav-current-*`, `--color-decorative-*`,
`--color-text-link`) that resolve to the accent by default and that the dimension re-points, so it
owns one layer of the token graph and knows no component. It never selects a brand, which is tier 1
and not a dimension. Sorted by *why* something is coloured: state and the primary action are
information and never dial; chrome and decoration are style and go first. The focus ring is its
invariant, the way the pill radius is the radius dimension's.
_Avoid_: brand strength, accent intensity, colorfulness, desaturation (it changes no colour)
...
generation input. Brand is not a selectable dimension, package entrypoint, or HTML attribute. *Accent
reach* is the dimension next to it and does not contradict this: brand is *which* ramps are loaded,
accent reach is *how far* they spread. `data-accent` selects the second, never the first.
