...
export function framedIn(moduleName: string) {
  ...
  return function framed<P extends object>(
    ...
    function Framed(props: P) {
       ...
       *
       * The stage stays empty until the reader selects React. A hidden binding must not create a
       * second document realm, React runtime, and component tree for every visible preview.
       *
       ...
       * `srcdoc` is no longer a React-controlled prop, so there is nothing for React to diff when
       * the reader selects its binding.
       *
