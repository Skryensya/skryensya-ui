export const annotationMessages = {
  es: {
    ...
    "annotation.numberedTitle": "Números en el margen, nombres abajo",
    "annotation.numberedBody":
      "Con <code>numbered</code>, cada burbuja lleva sólo su número y los nombres pasan a una lista ordenada debajo del marco. Es para el diagrama cuyos <em>nombres</em> son los que aprietan al sujeto: diez nombres de parte en los márgenes pueden dejarle un tercio del marco, y diez números le dejan casi todo. Lo que la leyenda numerada de siempre hace mal, obligar a sostener números en la cabeza mientras el ojo busca, acá no pasa: la guía y el anillo siguen ahí, y al pasar el puntero o enfocar una entrada de la lista se encienden juntos su número, su guía y su parte. El número es un contador de CSS, no texto: es la posición de la entrada, y una copia escrita a mano podría no coincidir.",
    "annotation.numberedLabel": "Anatomía de Accordion, numerada",
    ...
    "annotation.numberedPreviewLabel": "El mismo Accordion, con números y leyenda",
    ...
    "annotation.elbowBody":
      "Sólo hay tres direcciones posibles: recto sobre el eje, recto atravesándolo, y 45 grados. Con eso alcanza <strong>uno o dos tramos, siempre</strong>: si la distancia a lo largo alcanza para cubrir la del cruce, es un tramo recto y una diagonal; si no, es la diagonal primero y el resto recto. La demostración está en el comentario de <code>leaderPoints</code>. Aquí las cuatro piezas están una al lado de la otra, así que las cuatro etiquetas quieren el mismo milímetro del margen: el reparto las abre en abanico y cada guía tiene que doblar para volver. Es la única manera de <em>ver</em> el codo.",
    ...
    "annotation.optionsBody":
      "Del marco: <code>label</code> (nombra el diagrama y lo convierte en un <code>group</code>) <code>inert</code> (activado por defecto: el sujeto es una muestra, no toma puntero, ni foco, ni teclado) y <code>numbered</code> (números en los márgenes y los nombres en una leyenda debajo). De cada etiqueta: <code>for</code>, el selector que resuelve adentro del sujeto, y <code>side</code>, el margen que pide. Un selector inválido le cuesta su guía a esa etiqueta y a ninguna otra.",
  ...
  en: {
    ...
    "annotation.numberedTitle": "Numbers in the margin, names below",
    "annotation.numberedBody":
      "With <code>numbered</code>, each bubble carries only its number and the names move to an ordered list under the frame. It is for the diagram whose <em>names</em> are what squeeze the subject: ten part names in the margins can leave it a third of the frame, where ten numbers leave it nearly all of it. What the usual numbered legend gets wrong, making the reader hold numbers in their head while the eye searches, does not happen here: the leader and the ring are still drawn, and hovering or focusing a list entry lights its number, its leader and its part together. The number is a CSS counter, not text: it is the entry's position, and a hand-typed copy could disagree with it.",
    "annotation.numberedLabel": "Accordion anatomy, numbered",
    ...
    "annotation.numberedPreviewLabel": "The same Accordion, with numbers and a legend",
    ...
    "annotation.optionsBody":
      "On the frame: <code>label</code> (names the diagram and makes it a <code>group</code>) <code>inert</code> (on by default: the subject is a specimen, no pointer, no focus, no keyboard) and <code>numbered</code> (numbers in the margins, the names in a legend below). On each label: <code>for</code>, the selector resolved inside the subject, and <code>side</code>, the margin it asks for. An invalid selector costs that label its leader and no other.",
