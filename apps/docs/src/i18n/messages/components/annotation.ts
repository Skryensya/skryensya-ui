export const annotationMessages = {
  es: {
    ...
    "annotation.optionsBody":
      "Del marco: <code>label</code> (nombra el diagrama y lo convierte en un <code>group</code>) e <code>inert</code> (activado por defecto: el sujeto es una muestra, no toma puntero, ni foco, ni teclado). De cada etiqueta: <code>for</code>, el selector que resuelve adentro del sujeto, y <code>side</code>, el margen que pide. Un selector inválido le cuesta su guía a esa etiqueta y a ninguna otra.",
