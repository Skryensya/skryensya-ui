export const componentPreviewMessages = {
  es: {
    ...
    "componentPreview.screenBody6":
      'La elección es <strong>del preview, no del documento</strong>, y ahí se diferencia de la preferencia Vanilla | React: cambiarla en un ejemplo no toca a los demás. Hay una preferencia compartida igual, que vive en <code>&lt;html data-sk-component-preview-screen-pref&gt;</code> y se guarda en <code>localStorage["sk"].screen</code>, así que sobrevive a recargas y a navegar entre páginas; es la que siembra a todos los previews al cargar. Lo que no hace un click suelto es escribirla. <em>Libre</em> es la ausencia del atributo, no un tercer valor. Un click normal, en cambio, es <strong>sólo de ese preview</strong>: mueve ese stage y ninguno más, y no toca la preferencia guardada. Cambiar el preset suele ser una pregunta sobre una demo («¿esta tabla sobrevive en un teléfono?»), y responderla moviendo todos los demás previews tira a la basura donde el lector los había dejado. <kbd class="sk-kbd">Shift</kbd> + click es la difusión deliberada: ahí sí cambian todos y la elección se guarda. Cuesta un modificador porque es el gesto que pisa las cajas de los demás, y alcanza a los previews que el lector ya había movido a mano: es justamente la forma de volver a poner la página en fila.',
  ...
  en: {
    ...
    "componentPreview.screenBody6":
      'The choice belongs <strong>to the preview, not the document</strong>, and that is where it parts company with the Vanilla | React preference: changing it in one example leaves the others alone. There is still a shared preference, living on <code>&lt;html data-sk-component-preview-screen-pref&gt;</code> and saved in <code>localStorage["sk"].screen</code>, so it survives reloads and navigating between pages; it is what seeds every preview on load. What a plain click does not do is write to it. <em>Free</em> is the absence of the attribute, not a third value. An ordinary click, by contrast, is <strong>that preview&rsquo;s alone</strong>: it moves that stage and no other, and never touches the saved preference. Changing the preset is usually a question asked of one demo ("does this table survive a phone?"), and answering it by moving every other preview throws away wherever the reader had left them. <kbd class="sk-kbd">Shift</kbd> + click is the deliberate broadcast: that one moves everything and is saved. It costs a modifier because it is the gesture that overwrites other people&rsquo;s boxes, and it reaches previews the reader had already nudged by hand, which is exactly how you put the page back in step.',
    "componentPreview.screenBody7":
