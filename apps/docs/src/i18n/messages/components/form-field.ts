export const formFieldMessages = {
  es: {
    ...
    "formFieldPage.independentBody":
      "<code>sk-form-field</code> es el cromo alrededor de <em>cualquier</em> control: envuelve un select o un textarea igual de bien, como aquí abajo. Nombrarlo por el control que más veces sostiene lo volvería mentira la primera vez que sostenga otro: y por eso su slot <code>children</code> acepta cualquier signature y el cableado apunta a <code>\"control\"</code> en vez de a un input.",
    "formFieldPage.errorTitle": "El error es texto, no un color",
    "formFieldPage.errorBody":
      "La <em>presencia</em> del mensaje es lo que invalida el campo: no hay una opción <code>invalid</code> aparte que pueda quedar desfasada de él. <code>--sk-form-field-error-fg</code> tiñe un mensaje que igual tiene que existir, y <code>aria-invalid</code>, que ya escribes para los lectores de pantalla, es lo que sigue el hook <code>--sk-input-border-color</code>. El color nunca es la única señal de error (WCAG 1.4.1).",
    ...
    "formFieldPage.reactBody":
      "En React, <code>FormField</code> hace el cableado que en markup escribes a mano: genera el <code>id</code>, arma el <code>aria-describedby</code> de la ayuda y del error, y pasa <code>required</code> y <code>disabled</code> al control nativo. Se importa desde <code>@skryensya/react/form-field</code>, su propio módulo, y cualquier control puede leer ese contexto: que es la alternativa a que cada uno se haga su copia del cableado.",
