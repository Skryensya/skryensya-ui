...
summary: >-
  ...
  entre tiers es una cascade layer, y el sistema envía styling hooks, no componentes.
---
...
`components/button.css` se scopea a `.ds-button`, nunca a `:root`. Un consumidor que no envía botones
...
exactamente el caso que lo justifica, y se busca `--ds-button-bg` como contrato público.
...
codificación ingenua le da a cada uno su propia variable, `--ds-button-primary-bg`,
`--ds-button-danger-bg`, que es una explosión combinatoria entre la que el CSS del consumidor luego
...
```css
.ds-button                         { --ds-button-bg: var(--color-action-neutral); }
.ds-button[data-variant="primary"] { --ds-button-bg: var(--color-action-primary); }
```

El consumidor escribe `background: var(--ds-button-bg)` una vez y nunca ramifica.

Eso también hace el override externo trivial y total: `.hero .ds-button { --ds-button-bg: rebeccapurple }`
...
Styling hooks, no componentes. La app consumidora escribe el CSS estructural y los conecta. La única
excepción, y sus razones, está en [qué envía tier 3](/decisiones/0008-que-envia-tier-3).
