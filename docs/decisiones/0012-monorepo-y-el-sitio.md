...
- **`packages/core`**, tokens, styling hooks y contrato compartido de componentes. Cero dependencias de
  runtime, publicable; la carpeta `css/` es el [artefacto](/decisiones/0006-css-puro-sin-build),
  `src/` publica parts/tipos compartidos y `scripts/lint.mjs` es el
  [validador](/decisiones/0007-el-validador). El tooling del monorepo (turbo, pnpm) vive en la raíz y en
...
había sido exportado. El CSS se enviaba y [el state layer](/decisiones/0009-el-state-layer) lo trata como
...
costo cambió con ADR-13: **el contrato público ahora exige un bundler con Sass**. Eso es deliberado, el
...
lo que invierte el consejo de [la primera decisión](/decisiones/0001-tres-tiers-y-la-direccion-de-las-referencias) de
...
token tiene que superar la puerta de contraste del [validador](/decisiones/0007-el-validador) sobre dos
marcas y cuatro modos, el primer cambio lo bastante grande como para que la
[brecha conocida](/decisiones/0007-el-validador) golpee.
