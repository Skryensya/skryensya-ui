...
summary: >-
  El sidebar tenía un ds-sidebar__link, y el navbar tenía el suyo: las mismas reglas escritas dos veces,
...
El sidebar enviaba `ds-sidebar__list`, `ds-sidebar__item` y `ds-sidebar__link`. El navbar enviaba
`ds-navbar__list`, `ds-navbar__item` y `ds-navbar__link`. Las dos listas eran la misma idea escrita
...
nunca al inquilino. `ds-sidebar__link` nombra al inquilino: describe *dónde está parada* la lista, no
...
.ds-sidebar[data-state="collapsed"] .ds-nav-list__label { opacity: 0 }
```

Lo correcto es al revés: el shell **re-declara los hooks del pattern**, y la cascada los baja.

```css
.ds-sidebar[data-state="collapsed"] {
  --ds-nav-list-label-opacity: 0;
  --ds-nav-list-link-padding-x: calc((var(--size-control-sm) - var(--size-icon-md)) / 2);
}
```
...
su intención de motion (`--ds-nav-list-duration`), así los labels se desvanecen en el mismo tiempo en
...
- *Un part de icono en la lista.* El icono es un pattern y trae su propia caja (decisión 15). Un slot
  aquí sería la lista re-declarando lo que `ds-icon` ya envía.
...
Y hay una migración: `ds-sidebar__link` y `ds-navbar__link` ya no existen. No hay alias y no hay
