...
package of the [skryensya-design-system](../../README.md) Turborepo; it's about the *engineering* of tokens
...
packages/core/            ← this package (tokens + shared contract)
  css/
    ...
    primitives/_ramps.scss  root seed defaults + derived ramp recipe
    ...
    modes/hc.scss           opt-in Sass high-contrast: overrides tier-2 colour
...
`tokens.scss` is the complete token entrypoint. It ships one root ramp set; consumers configure it by
overriding seed custom properties in their own `:root` rule. No brand import, selector, or HTML attribute
...
of source order. A consumer's unlayered CSS beats every layer, so this is sufficient to configure identity:
...
The seed values feed the derived ramps at computed-value time. `--seed-danger`, `--seed-success` and
`--seed-warning` have sober defaults and are optional overrides; an app may also override a specific
`--ramp-*` stop. Semantic tokens keep referencing role positions, never hues, so `accent-600` has one
meaning regardless of the configured identity.

## Four dimensions that compose

| Dimension | Owns | Mechanism | File |
|-----------|------|-----------|------|
| **Colour mode** | tier-2 colour | `light-dark()` picks a slot | `semantic.scss` |
| **High contrast** | tier-2 colour | override block re-declares the same tokens | `modes/hc.scss` |
| **Density** | tier-2 spacing | runtime `--ds-density` multiplier | `semantic.scss` |
...
to any element that opts in with `class="ds-interactive"`. It tints with `currentColor`, so the same mechanism
works on neutral surfaces and saturated fills in both colour modes. Exactly one layer is active at a time;
...
## One root brand, configured through seeds

The package intentionally ships one ramp configuration, derived from five seed custom properties. Two are
the identity inputs: `--seed-accent` is the 600 rung for the primary ramp and `--seed-neutral` gives greys
their undertone. The remaining signal seeds have safe defaults. `tokens.scss` includes the complete ramp set,
and unlayered consumer CSS overrides the seeds without changing markup:
...
The validator measures the shipped defaults. Consumers own the accessibility check for the values they choose,
just as they own any unlayered override.
