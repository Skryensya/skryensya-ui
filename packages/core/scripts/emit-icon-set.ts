...
export function generateIconSet({ dir, label, map, attrs, header, resolve, argv = process.argv }: IconSetConfig) {
  ...
  if (argv.includes("--check")) {
    ...
    if (stale.length > 0) {
      ...
      console.error(`✗ ${files.join(", ")} out of date, run \`pnpm generate\``);
      process.exit(1);
    }
    console.log(`✓ ${label}: ${count} roles and the Carousel adapter are up to date`);
  } else {
    writeFileSync(setTarget, setOut);
    writeFileSync(carouselTarget, carouselOut);
    console.log(`✓ ${label}: ${count} roles and the Carousel adapter generated`);
  }
}
