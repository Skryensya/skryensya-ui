...
export const stateButtonParts = {
  root: "sk-state-button",
} as const;

export type StateButtonPart = keyof typeof stateButtonParts;
export type StateButtonPartClass = (typeof stateButtonParts)[StateButtonPart];

export const stateButtonAttrs = {
  root: "data-sk-state-button",
  current: "data-current",
  face: "data-face",
} as const;

export type StateButtonAttr = keyof typeof stateButtonAttrs;
export type StateButtonAttrName = (typeof stateButtonAttrs)[StateButtonAttr];
...
export const stateButtonContract = {
  id: "state-button",
  category: "actions",
  css: "@skryensya/core/components/state-button.css",
  parts: stateButtonParts,
  hooks: [
    "--sk-state-button-rest-blur",
    "--sk-state-button-rest-scale",
    "--sk-state-button-transition-duration",
    "--sk-state-button-transition-easing",
  ],

  options: {
    /** Which face's `name` is current. Absent means no face carries `data-active` at all. */
    current: { type: "string", attr: stateButtonAttrs.current, keyOf: { slot: "faces" } },
  },
  ...
  signatures: {
    StateButton: {
      ...
      slots: {
        ...
        faces: {
          ...
          item: {
            ...
            requires: ["icon"],
            options: {
              name: { type: "string", attr: stateButtonAttrs.face },
      ...
      react: { from: "@skryensya/react/state-button", name: "StateButton" },
    },
  },
  ...
  a11y: [
    {
      ...
      because:
        "StateButton is always icon-only: faces are glyphs with no visible text, so the host owns the accessible name.",
    },
  ],
