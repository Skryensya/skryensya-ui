 ...
 * non-colour cue fails WCAG 1.4.1 (see core/css/components/typography.css). Hover/press/focus come
 * from `ds-interactive` + the state layer, same as every other control. Default paint matches prose;
 * pass `tone="primary"` for the brand-coloured call-out. */
export function Link({ children, className, tone, ...props }: LinkProps) {
  return (
    <a
      {...props}
      className={cx(`${typographyParts.link} ds-interactive`, className)}
      data-tone={tone}
    >
      {children}
    </a>
