...
describe("TablePager Vanilla contracts", () => {
  ...
  it("moves pages on item click and resets when the size select changes", () => {
    const root = mount(markup);
    ...
    // A native select, or the hidden one both Select bindings keep and Zag fires `change` on.
    const select = root.querySelector<HTMLSelectElement>(".sk-table-pager__size select")!;
    select.value = "5";
  ...
  it("pages every body row of a table composed with no marker attributes", () => {
    const root = mount(markup);
    mountTablePager(root);
    expect(root.querySelectorAll("tbody tr[hidden]")).toHaveLength(3);
  });

  it("also hears an enhanced Select's own event from the size part", () => {
    const root = mount(markup);
    mountTablePager(root);
    root
      .querySelector(".sk-table-pager__size")!
      .dispatchEvent(new CustomEvent("sk:selectvaluechange", { bubbles: true, detail: { value: ["5"] } }));
    expect(root.getAttribute("data-page-size")).toBe("5");
  });
});
