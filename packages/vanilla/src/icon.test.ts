...
describe("mountIcons", () => {
  it("replaces a placeholder with the bound set's svg", () => {
    const { count, icon } = mount(`<span data-sk-icon="arrow-up"></span>`);

    expect(count).toBe(1);
    expect(document.body.querySelector("[data-sk-icon]")).toBeNull(); // the placeholder is gone
    expect(icon).not.toBeNull();
    expect(icon!.getAttribute("class")).toBe("sk-icon");
  ...
  it("preserves the author's extra classes, always keeping sk-icon", () => {
    const { icon } = mount(`<span data-sk-icon="arrow-up" class="text-up"></span>`);
    expect(icon!.getAttribute("class")).toBe("sk-icon text-up");
  });

  it("carries the author's other attributes (style, id) onto the svg", () => {
    const { icon } = mount(`<span data-sk-icon="arrow-up" id="up" style="color: green" title="Sube"></span>`);
    ...
    expect(icon!.hasAttribute("data-sk-icon")).toBe(false);
  });

  it("maps data-sk-icon-size to data-size", () => {
    const { icon } = mount(`<span data-sk-icon="close" data-sk-icon-size="sm"></span>`);
  ...
  it("leaves an unknown role intact and warns once", () => {
    ...
    document.body.innerHTML = `<span data-sk-icon="warehouse"></span><span data-sk-icon="warehouse"></span>`;

    const count = mountIcons(document.body, set);

    expect(count).toBe(0);
    expect(document.body.querySelectorAll("[data-sk-icon]").length).toBe(2); // untouched
  ...
  it("hydrates the root element itself when it is a placeholder", () => {
    ...
    el.setAttribute("data-sk-icon", "arrow-up");
  ...
  it("is idempotent, a second pass finds no placeholders", () => {
    document.body.innerHTML = `<span data-sk-icon="arrow-up"></span>`;
